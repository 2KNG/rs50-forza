...
## ⚡ 집 도착 직후 체크리스트 (이 순서 그대로)

1. 휠 전원 ON, **정중앙 잡고** `python tools\recenter.py --set`
2. **베이스 전원 껐다 켜기** (설정 반영에 재부팅 필요한 사례 다수)
3. G HUB 열기: 회전 900° 확인 (강도 8.0Nm은 이미 설정됨) — **G HUB는 켜둔 채 진행**
   (종료는 나중에 TF4ALL 도입 시에만)
4. **휠 켜진 상태에서** 게임 실행 (휠 없이 실행하면 프로파일 초기화!)
5. 게임 내 고급 휠 설정에 아래 "2." 권장값 입력
6. **앱 없이 2~3분 주행** — 느낌 정상이면 7로, 이상하면 아래 "3." 분기
7. `start.bat` → 플레이 (변속/LED/대시보드 전부 자동)

도구가 미발견/Timeout으로 죽으면: 휠을 살짝 움직여 깨우거나 베이스 전원 재인입 → 10초 후 재실행.
...
(가벼움 지속 시 에스컬레이션 순서: ① 게임 FFB Scale 1.5로 ② Device 1 바인딩 재확인 ③ 프로파일 폴더 백업+삭제):
