...
| `http://127.0.0.1:8777/config` | 설정 전용 창 (두 화면에 즉시 반영) |
...
## 화면 커스터마이즈 — 독립 CONFIG 창

`start.bat`이 대시보드와 함께 **설정 전용 창**(`/config`)을 하나 띄웁니다.
평소엔 뒤에 깔려 있다가 **Alt+Tab**으로 불러 쓰면 되고, **변경은 좌/우 두
화면에 즉시 반영**됩니다(BroadcastChannel 동기화). 덕분에 대시보드 화면엔
설정 UI가 전혀 남지 않습니다 — 계기판만 꽉 채웁니다.

| 항목 | 옵션 | 설명 |
|---|---|---|
| 테마 | PIT · **GT** · F1 · RETRO · OLED · NEON · **CLASSIC** · **VFD** | GT = GT7 럭셔리 클러스터 / CLASSIC = 50~70년대 영국 클래식(Smiths 크림 다이얼+검정 니들) / VFD = 80년대 디지털 계기판(505nm 시안 발광) |
| 숫자 폰트 | AA · DIN · 01 · AGY · IMP · BLK · EURO · ORBIT | DIN=Bahnschrift(실차 표준 DIN), AGY=Agency FB, IMP=Impact, EURO=Michroma(유로스타일 계열), ORBIT=Orbitron |
| 표시 모드 | DIG · ANA | 디지털 숫자판 ↔ 아날로그 게이지 |
| rev 바 | BAR ▲ · BAR ▲▼ | ▲▼ = 화면 하단에도 rev 바 추가 |
| 바 스타일 | SEG · FIRE | FIRE = 일렁이는 화염 실루엣 |
| G 이펙트 | GFX · GFX✕ | 화면 테두리 G-포스 비네트 |

오버레브(변속 시점) 도달 시 전체 스트립이 보라색으로 8Hz 점멸하고
화면 테두리가 함께 번쩍입니다. 변속 순간엔 이전 기어 숫자가 확대·페이드아웃.

URL 파라미터로 직접 지정도 가능:
`/left?th=gt&fn=din&bar=both&fx=flame&dsp=analog&gv=on`

## 주행 감각 강화 요소

| 요소 | 동작 |
|---|---|
| **G-포스 비네트** | 브레이킹=상단 적색, 가속=하단 녹색, 코너링=관성 방향 사이드 글로우 (0.35G부터, 1.5G 포화) |
| **드리프트 스코어러** | 각도×속도×시간 누적, 콤보 배수 1.0~5.0 (지속 +0.1/s, 좌우 전환 +0.5), 스핀·언더스티어는 DIRTY 소멸, 직진 2초에 BANK 적립 + 세션 베스트 영구 기록 |
| **등급 콜아웃** | NICE 2k → GREAT 6k → AWESOME 12k → ULTIMATE 20k 배너 |
| **주행라인 맵** | 드리프트각으로 색칠 (그립 파랑 → 한계 빨강, 각도 클수록 굵게) |
| **타이어 한계 펄스** | 복합 슬립 2 이상인 바퀴 카드가 적색 발광 |
| **TRIP 적산계** | 세션 주행거리 |

## 왜 부드러운가

- 서버가 **SSE(`/events`)로 30Hz 중앙 푸시**합니다 — 좌/우/설정 창이 전부
  같은 프레임을 같은 순간에 받으므로 창마다 값이 어긋나지 않습니다.
  (SSE 불가 시 150ms 폴링으로 자동 폴백. `?stream=off`로 폴링 강제 —
  헤드리스 캡처는 열린 SSE 연결에서 멈추므로 검증용으로 필요)
- 오버레브 점멸은 `Date.now()` 벽시계 기준이라 **모든 창이 같은 위상**으로
  깜빡입니다. 기본 4Hz.
- 표시는 **60fps 보간**입니다. RPM/속도/G/드리프트각은
  지수 스무딩(무게감), 페달·조향·핸드브레이크·타이어·좌표는 **스냅샷 보간**
  (폴링 간격을 측정해 이전값→새값을 선형 이동)으로 처리합니다.
- 스냅샷 보간을 쓰는 이유: 지수 스무딩은 시정수가 지나면 평탄해져 폴링 주기마다
  계단이 남지만, 이 방식은 다음 샘플이 도착할 때까지 계속 움직여 페달·조향
  트레이스가 완전히 이어진 곡선이 됩니다.
...
| `[led]` | `start_ratio` | rev 바 점등 시작 rpm 비율 (기본 0.35 — 낮출수록 자주 차오름) |
